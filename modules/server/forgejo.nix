{ config, pkgs, lib, ... }: {
  ...
  options.stages.server.services.forgejo = {
  ...
  config = let
    cfg = config.stages.server.services.forgejo;
  in lib.mkIf cfg.enable (lib.mkMerge [
  (lib.mkIf (if cfg.user == "forgejo" then false else true) {
    users.users.git = {
  ...
  {
    # uses the hosts defaultSopsFile. should be changed at some point
    sops.secrets = let
      gitSecret = {
        owner = cfg.user;
        group = cfg.user;
